const printOrder = (order) => {
  console.log(
    `${order.customer.name}, bought a ${order.instrument.brand} ${order.instrument.model} ${order.instrument.kind} ${order.instrument.category} from ${order.instrument.shop.name}`
  )
}
